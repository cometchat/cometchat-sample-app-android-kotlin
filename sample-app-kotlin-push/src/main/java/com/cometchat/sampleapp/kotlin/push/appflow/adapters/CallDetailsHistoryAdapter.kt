package com.cometchat.sampleapp.kotlin.push.appflow.adapters
...
import com.cometchat.sampleapp.kotlin.push.databinding.CallDetailsHistoryItemsBinding
...
class CallDetailsHistoryAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (callLog.type == CometChatCallsConstants.CALL_TYPE_AUDIO ||
            ...
            when {
                isLoggedInUser -> {
                    holder.binding.tvInfoTitle.setText(com.cometchat.sampleapp.kotlin.push.R.string.app_call_outgoing)
                    holder.binding.tvInfoTitle.setTextColor(CometChatTheme.getTextColorPrimary(context))
                    setupCallIcon(
                        holder.binding.ivInfoIcon,
                        AppCompatResources.getDrawable(context, R.drawable.cometchat_ic_outgoing_call),
                        CometChatTheme.getSuccessColor(context)
                    )
                }
                isMissedOrUnanswered -> {
                    holder.binding.tvInfoTitle.setText(com.cometchat.sampleapp.kotlin.push.R.string.app_call_missed)
                    holder.binding.tvInfoTitle.setTextColor(CometChatTheme.getErrorColor(context))
                    setupCallIcon(
                        holder.binding.ivInfoIcon,
                        AppCompatResources.getDrawable(context, R.drawable.cometchat_ic_missed_call),
                        CometChatTheme.getErrorColor(context)
                    )
                }
                else -> {
                    holder.binding.tvInfoTitle.setText(com.cometchat.sampleapp.kotlin.push.R.string.app_call_incoming)
                    holder.binding.tvInfoTitle.setTextColor(CometChatTheme.getTextColorPrimary(context))
                    setupCallIcon(
                        holder.binding.ivInfoIcon,
                        AppCompatResources.getDrawable(context, R.drawable.cometchat_ic_incoming_call),
                        CometChatTheme.getSuccessColor(context)
                    )
                }
            }
