# CometChat Android (Jetpack Compose) UIKit — E2E Test Suite

End-to-end (E2E) instrumented tests for the **CometChat Android Compose UIKit** as consumed by the
**`sample-app-compose`** app (the **Jetpack Compose** UI). The tests drive the real app on an
...
> There is a parallel, identical-by-design suite for the classic **View** app in
> [`sample-app-kotlin`](../sample-app-kotlin/E2E_README.md).
...
```
sample-app-compose/src/androidTest/java/com/cometchat/sampleapp/compose/e2e/helpers/E2ETestConfig.kt
```
...
./sample-app-compose/run_e2e_tests.sh
...
./sample-app-compose/run_e2e_tests.sh \
  com.cometchat.sampleapp.compose.e2e.realtime.RealTimeMessagingTest#test02_receiveText
```
...
3. **Build & run** — runs `:sample-app-compose:connectedDebugAndroidTest`, streaming **live per-test
...
└── src/androidTest/java/com/cometchat/sampleapp/compose/e2e/
...
Reports are written by AGP under `sample-app-compose/build/`:
...
| Assert | JUnit `assert*` over UIAutomator finders | Verifies the UI. Compose surfaces content descriptions (e.g. receipts → `Message sent/delivered/read`) that the View UI draws on canvas. |

Run order within a class is fixed (`@FixMethodOrder(NAME_ASCENDING)`), so tests are named
`test01_…`, `test02_…`.

**Compose-specific note:** the Compose message list does **not** auto-scroll to newly received
messages the way the View list does, so the polling helpers scroll while waiting. Receipt state,
presence, and the unread divider are exposed as content descriptions in Compose (unlike the
canvas-drawn View equivalents), so several assertions are stronger here.
...
**1-to-1 / shared (Compose)** — `AuthenticationE2ETest`, `ConversationsE2ETest`, `UsersE2ETest`,
...
> **Known gap — incoming calls.** `RealTimeCallE2ETest` exercises the **incoming-call** component
> (the partner initiates a call via `CometChatJsDriver`). The Compose sample app does not yet wire up
> the incoming-call screen, so these tests are present for parity but will not pass until that app
> feature lands. The View app (`sample-app-kotlin`) does wire it, where the same tests pass.
...
| Output | Path |
|--------|------|
| HTML report (open in a browser) | `sample-app-compose/build/reports/androidTests/connected/debug/index.html` |
| JUnit XML (per-class) | `sample-app-compose/build/outputs/androidTest-results/connected/debug/` |
| Run log (full console output) | `sample-app-compose/e2e-results/run_<timestamp>.log` |
...
| Received message not found while chat is open | The Compose message list doesn't auto-scroll to new messages. | Use the polling helpers (they scroll); already handled in the suite. |
| `RealTimeCallE2ETest` fails | The Compose sample app has no incoming-call screen yet (see §7). | Expected until the app feature is added; run the View suite for call coverage. |
