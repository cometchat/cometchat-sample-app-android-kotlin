...
/**
 * Shared base for single-emulator REST-driven real-time tests (sample-app-compose).
 *
 ...
 *
 * Compose note: the message list only auto-scrolls to a new message when already pinned at the
 * bottom (CometChatMessageList: `if (scrollToBottomOnNewMessage && isAtBottom) …`), so
 * [pollForMessageInChat] scrolls toward the bottom each iteration to surface new messages.
 *
...
abstract class RealtimeTestBase {
    ...
    /** Polls until any on-screen node contains [snippet] (no scrolling). */
    ...
    /**
     * Polls for [snippet] in the open chat, scrolling toward the bottom each iteration so a
     * just-arrived message (which Compose only auto-scrolls to when already at bottom) is
     * brought into the rendered viewport.
     */
    ...
    /** Polls until ANY of [snippets] appears in the open chat, scrolling toward the bottom. */
    ...
    /**
     * Polls for [snippet] in the conversation list preview, scrolling toward the top each
     * iteration (a new message moves the conversation to the top of the virtualized list).
     */
    ...
    /**
     * Ensures the latest message's reaction chip is visible. On Compose the chip on the last
     * message can sit behind the composer, so nudge the list up to reveal it.
     */
    protected fun revealLatestForReaction() {
        E2ETestHelper.scrollUp(device)
    }
