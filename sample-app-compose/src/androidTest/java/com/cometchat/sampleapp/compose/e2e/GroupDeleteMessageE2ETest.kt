...
/**
 * E2E tests for deleting messages in a group conversation (sample-app-compose).
 *
 ...
 * - Uses retry loops and coordinate-based clicks (StaleObjectException)
 ...
 *       -Pandroid.testInstrumentationRunnerArguments.class=com.cometchat.sampleapp.compose.e2e.GroupDeleteMessageE2ETest
 */
...
class GroupDeleteMessageE2ETest {
    ...
    fun setup() {
        ...
        // Re-launch the app to ensure UI is in sync after SDK login switches
    ...
    fun test03_adminDeletesOtherMemberMessage() {
        ...
        longPressMessageCompose("OtherMsg")
        Thread.sleep(2000)
    ...
    fun test04_regularMemberCannotDeleteOthersMessage() {
        ...
        E2ETestHelper.scrollDown(device)
        Thread.sleep(1000)
        var targetMsg = device.findObject(By.textContains(adminMsg))
        if (targetMsg == null) {
            E2ETestHelper.scrollUp(device)
            Thread.sleep(2000)
            targetMsg = device.findObject(By.textContains(adminMsg))
        }

        // Long-press the admin's message
        longPressMessageCompose(adminMsg)
        Thread.sleep(2000)
    ...
    /**
     * Long-presses a message in the Compose app using coordinate-based swipe.
     * This avoids StaleObjectException by capturing bounds immediately.
     */
