...
object PinSaveRegressionHelper {
    ...
    fun openChatWith(device: UiDevice, uid: String = E2ETestConfig.ONE_TO_ONE_UID) {
        ...
        tap(device, row!!)
        assertTrue("Message composer not shown", device.wait(Until.hasObject(By.clazz("android.widget.EditText")), TIMEOUT))
    ...
    /** Long-presses the bubble showing [text]: longClick first (Compose), then shell long-touch. */
    fun longPress(device: UiDevice, text: String) {
        device.findObject(By.textContains(text))?.longClick()
        Thread.sleep(2500)
        if (!isAnyOptionVisible(device)) {
            val b = device.findObject(By.textContains(text))?.visibleBounds ?: return
            device.executeShellCommand("input touchscreen swipe ${b.centerX()} ${b.centerY()} ${b.centerX()} ${b.centerY()} 1500")
            Thread.sleep(2000)
        }
    }
