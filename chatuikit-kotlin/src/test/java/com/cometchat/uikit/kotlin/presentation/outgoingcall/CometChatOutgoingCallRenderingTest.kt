...
/**
 * Tests for CometChatOutgoingCall Kotlin component rendering states.
 *
 * Verifies that the ViewModel state drives the correct rendering behavior:
 * - call == null → component is in idle/hidden state
 * - call != null → component shows receiver info (name, avatar, call type)
 * - endCallButtonEnabled == true → end call button is enabled
 ...
 *
 * Run with:
 *   ./gradlew :chatuikit-kotlin:testDebugUnitTest --tests "*CometChatOutgoingCallRenderingTest"
 */
...
class CometChatOutgoingCallRenderingTest : FunSpec({
    ...
    test("ViewModel initial state → component is in idle state (no call to display)") {
        runTest {
            ...
            println("    ✅ Initial state: call=null, endCallButtonEnabled=true → component idle/hidden")
        }
    }

    // ==================== Ringing State Rendering (Requirement 9.2) ====================

    test("ViewModel with call set → component shows receiver name and call type") {
        runTest {
            val viewModel = CometChatOutgoingCallViewModel(enableListeners = false)
            val call = createMockCall(
                sessionId = "session-render-1",
                type = "audio",
                callerUid = "receiver-1",
                callerName = "Alice"
            )
            ...
            println("    ✅ Call set: receiver=Alice, type=audio → component renders receiver info")
        }
    }
    ...
    // ==================== End Call Button Enabled State (Requirement 9.6) ====================

    test("ViewModel endCallButtonEnabled starts true → end call button is enabled") {
        runTest {
            val viewModel = CometChatOutgoingCallViewModel(enableListeners = false)
            advanceUntilIdle()
            ...
            println("    ✅ endCallButtonEnabled=true initially → button enabled")
        }
    }

    test("ViewModel setCall resets endCallButtonEnabled to true") {
        runTest {
            val viewModel = CometChatOutgoingCallViewModel(enableListeners = false)
            val call = createMockCall(sessionId = "reset-enabled")
            ...
            println("    ✅ setCall resets endCallButtonEnabled=true")
        }
    }

    // ==================== Terminal State Rendering (Requirement 9.3, 9.4) ====================

    test("ViewModel with acceptedCall not set → component stays in calling state") {
        runTest {
            val viewModel = CometChatOutgoingCallViewModel(enableListeners = false)
            ...
            println("    ✅ acceptedCall=null → component stays in calling state")
        }
    }

    test("ViewModel with rejectedCall not set → component stays in calling state") {
        runTest {
            val viewModel = CometChatOutgoingCallViewModel(enableListeners = false)
            ...
            println("    ✅ rejectedCall=null → component stays in calling state")
        }
    }
    ...
    test("PBT: for any receiver name → ViewModel exposes correct name for rendering") {
        checkAll(30, Arb.string(3..20), Arb.string(5..15)) { receiverName, sessionId ->
            runTest {
                ...
                println("    → receiverName=$receiverName → correct name exposed")
            }
        }
        println("    ✅ PBT: All receiver names render correctly")
    }

    // ==================== State Transition Rendering ====================

    test("switching calls → component re-renders with new receiver info") {
        runTest {
            val viewModel = CometChatOutgoingCallViewModel(enableListeners = false)
            ...
            println("    ✅ Switching calls: Alice(audio) → Bob(video) → re-renders correctly, endCallButtonEnabled reset")
        }
    }
})
