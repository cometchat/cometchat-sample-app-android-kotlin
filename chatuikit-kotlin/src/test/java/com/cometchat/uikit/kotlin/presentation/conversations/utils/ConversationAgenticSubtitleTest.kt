...
/**
 * Robolectric tests for [ConversationUtils.getLastMessageText] across the card +
 * AI-agent contexts (ENG-36415, task 1.7 — conversation-list preview).
 *
 * Verifies the conversation-list subtitle for:
 * - an `agentic` (AI assistant) message — uses its text, else the localized
 *   "AI agent message" label (the agent-in-group / BYOA last-message preview), and
 * - a developer card — uses its text, else "Card Message", identically in 1-1 and group.
 *
 * Run with:
 *   ./gradlew :chatuikit-kotlin:testDebugUnitTest --tests "*ConversationAgenticSubtitleTest"
 */
