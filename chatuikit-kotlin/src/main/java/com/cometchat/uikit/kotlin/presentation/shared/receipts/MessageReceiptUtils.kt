...
object MessageReceiptUtils {
     ...
     * @return The appropriate [ReceiptStatus]
     */
    private fun getReceipt(baseMessage: BaseMessage): ReceiptStatus {
        val moderationStatus = getModerationStatus(baseMessage)

        return when {
            moderationStatus == ModerationStatus.DISAPPROVED -> ReceiptStatus.ERROR
            baseMessage.id == 0L -> ReceiptStatus.IN_PROGRESS
            baseMessage.readAt != 0L -> ReceiptStatus.READ
            baseMessage.deliveredAt != 0L -> ReceiptStatus.DELIVERED
            baseMessage.sentAt > 0 || moderationStatus == ModerationStatus.PENDING -> ReceiptStatus.SENT
            else -> ReceiptStatus.IN_PROGRESS
        }
    }
     ...
     * - The message category is not MESSAGE or INTERACTIVE (unless metadata has incrementUnreadCount)
     *
     * @param baseMessage The message to check
