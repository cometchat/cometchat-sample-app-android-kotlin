...
/**
 * Callback interface for MediaRecorderManager events.
 * Uses the MediaRecorderState enum (IDLE, RECORDING, RECORDED).
 * 
 * This interface is different from [MediaRecorderCallback] which is used by the legacy
 * [CometChatMediaRecorder] component. This interface provides more detailed callbacks
 * for time updates, amplitude changes, and playback progress.
 */
interface MediaRecorderManagerCallback {
    /**
 ...
 *
 * This implementation matches the Jetpack Compose module's MediaRecorderManager for
 * consistent behavior across both UIKit implementations.
 *
 * Features:
 ...
 * manager.setCallback(object : MediaRecorderManagerCallback {
...
class MediaRecorderManager(private val context: Context) {
    ...
    private var callback: MediaRecorderManagerCallback? = null
     ...
     */
    fun setCallback(callback: MediaRecorderManagerCallback) {
