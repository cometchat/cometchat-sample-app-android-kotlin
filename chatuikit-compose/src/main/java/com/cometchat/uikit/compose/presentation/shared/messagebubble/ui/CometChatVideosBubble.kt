...
/**
 * Renders every video attachment of [message] as a grid with play badges. Part of the ENG-36737
 * per-type multi-attachment bubbles; see [MediaBubbleContent] for the shared grid.
 *
 * @param message The [MediaMessage] whose video attachments are rendered
 ...
 * @param onMediaClick Callback when a video is tapped (index, attachment)
...
fun CometChatVideosBubble(
    ...
    style: CometChatVideosBubbleStyle = when (alignment) {
        UIKitConstants.MessageBubbleAlignment.RIGHT -> CometChatVideosBubbleStyle.outgoing()
        else -> CometChatVideosBubbleStyle.incoming()
    },
    ...
    val videos = remember(message.id) { resolveAttachments(message) }
    val thumbnailUrl = remember(message.id) { message.thumbnailUrl() }
    MediaBubbleContent(
        videos, style.toGridAppearance(), isVideo = true, caption,
