...
/**
 * Integration tests for CometChatMentionsFormatter in chatuikit-jetpack (Compose).
 * 
 * Tests cover:
 ...
 * - AnnotatedString formatting for Compose
 * 
 * Based on design document sections 4.1, 4.2, 4.3 and section 10 (Compose-specific).
 * 
...
class CometChatMentionsFormatterIntegrationTest : DescribeSpec({
    ...
    describe("User Search API Integration") {
        ...
        describe("search with UsersRequest") {
            ...
            it("should create UsersRequest with search keyword") {
                val searchHelper = MentionSearchHelperCompose()
    ...
    describe("Message Send with Mentions") {
        ...
        describe("handlePreMessageSend processing") {
            ...
            it("should populate mentionedUsers on TextMessage") {
                val messageHelper = MentionMessageHelperCompose()
            ...
            it("should skip items with invalid data") {
                ...
                val messageHelper = MentionMessageHelperComposeWithValidation()
    ...
    describe("Edit Message with Mentions") {
        ...
        describe("existing mentions preservation") {
            ...
            it("should parse existing mentions from message text") {
                val spanHelper = MentionSpanHelperCompose()
    ...
    describe("Compose AnnotatedString Formatting") {

        describe("bubble span formatting") {

            it("should format left bubble mentions") {
                val spanHelper = MentionSpanHelperCompose()
                val user = createMockUser("user1", "John")
                val messageText = "Hello <@uid:user1>"
                val result = spanHelper.prepareBubbleSpan(messageText, listOf(user))
                result shouldBe "Hello @John"
            }

            it("should format right bubble mentions") {
                val spanHelper = MentionSpanHelperCompose()
                val user = createMockUser("user1", "John")
                val messageText = "Hello <@uid:user1>"
                val result = spanHelper.prepareBubbleSpan(messageText, listOf(user))
        ...
        describe("conversation span formatting") {

            it("should format conversation preview mentions") {
                val spanHelper = MentionSpanHelperCompose()
                val user = createMockUser("user1", "John")
                val messageText = "<@uid:user1> sent a message"
                val result = spanHelper.prepareConversationSpan(messageText, listOf(user))
                result shouldBe "@John sent a message"
            }
        }

        describe("mention all formatting") {

            it("should format mention all in bubble") {
                val spanHelper = MentionSpanHelperCompose()
...
/**
 * Helper class for testing mention search functionality in Compose.
 * Simulates the search behavior from CometChatMentionsFormatter.
 */
private class MentionSearchHelperCompose {
...
/** Helper class for testing message handling with mentions in Compose. */
private class MentionMessageHelperCompose {
    private val selectedMentions = mutableListOf<SuggestionItem>()
...
private class MentionMessageHelperComposeWithValidation {
    private val selectedMentions = mutableListOf<SuggestionItem>()
    
    fun addSelectedMention(item: SuggestionItem) { selectedMentions.add(item) }
    
    fun getMentionUsers(): List<User> = selectedMentions
        .filter { it.id.isNotEmpty() && it.data != null }
...
/** Helper class for testing span formatting in Compose. */
private class MentionSpanHelperCompose {
    ...
    fun prepareConversationSpan(text: String, mentionedUsers: List<User>): String {
        var result = text
        if (!disableMentionAll) result = result.replace(mentionAllPattern, "@$mentionAllLabelText")
        for (user in mentionedUsers) result = result.replace("<@uid:${user.uid}>", "@${user.name}")
        return result
