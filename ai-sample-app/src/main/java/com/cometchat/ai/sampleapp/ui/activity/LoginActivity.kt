...
import com.cometchat.ai.sampleapp.databinding.ActivityLoginBinding
import com.cometchat.ai.sampleapp.ui.adapters.SampleUsersAdapter
import com.cometchat.ai.sampleapp.viewmodels.LoginViewModel
...
class LoginActivity : AppCompatActivity() {
    ...
    private fun initClickListeners() {
        binding.btnContinue.setOnClickListener { v: View? ->
            if (viewModel.selectedUser.value == null && binding.etUid.text.toString().isEmpty()) {
                showToast(ContextCompat.getString(this, R.string.app_error_invalid_user_or_uid_login))
            } else {
    ...
    private fun onUserLogin(): Observer<Boolean> {
        return Observer { isLoggedIn: Boolean? ->
            if (isLoggedIn != null) {
                if (isLoggedIn) {
                    startActivity(Intent(this@LoginActivity, AIAssistantUsersActivity::class.java))
                    finish()
                } else {
                    showToast(ContextCompat.getString(this@LoginActivity, R.string.app_error_invalid_user_or_uid_login))
                }
            }
        }
    }
