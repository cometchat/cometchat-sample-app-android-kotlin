package com.cometchat.ai.sampleapp.compose.app
...
/**
 * Application class for the AI Assistant Sample App (v6 Compose UIKit).
 *
 * This sample demonstrates the AI Agent chat experience using CometChat v6
 * (`chatuikit-compose` + `chatuikit-core`). SDK initialization is deferred to
 * the credentials flow — not performed in [onCreate] — so users can switch
 * credentials and we avoid initializing with invalid placeholder values.
 *
 * Notes:
...
class AIAssistantApplication : Application() {
    ...
    companion object {
        private const val TAG = "AIAssistantAppCompose"
    ...
    override fun onCreate() {
        super.onCreate()
        Log.d(TAG, "AIAssistantApplication created (SDK init deferred)")
    }

    /**
     * Initializes the CometChat SDK with the provided credentials.
     *
     * Called from [com.cometchat.ai.sampleapp.compose.ui.credentials.AppCredentialsScreen]
     * (first-time setup) and [com.cometchat.ai.sampleapp.compose.ui.splash.SplashScreen]
