...
/**
 * Login screen — sample users grid + manual UID input + Continue button.
 *
 * Auto-navigates to the AI users screen if the SDK already reports a logged-in
 * user (handles activity restart / process-death cases).
 */
@Composable
fun LoginScreen(
    ...
    // Auto-login check: if user already logged in, skip the screen.
    LaunchedEffect(Unit) {
        try {
            if (CometChatUIKit.getLoggedInUser() != null) onLoginSuccess()
        } catch (_: Exception) { /* SDK not initialized yet */ }
    }
    ...
    LaunchedEffect(manualUid) {
        if (manualUid != manualUidText) manualUidText = manualUid
    }
    ...
    Column(
        ...
        if (users.isNotEmpty()) {
            ...
            Row(
                ...
                HorizontalDivider(modifier = Modifier.weight(1f), thickness = 1.dp, color = colorScheme.strokeColorDefault)
                Text(
        ...
        BasicTextField(
            ...
            modifier = Modifier
                .fillMaxWidth()
                .padding(top = 8.dp)
                .background(color = colorScheme.backgroundColor2, shape = RoundedCornerShape(8.dp))
                .padding(8.dp)
                .onFocusChanged { if (it.isFocused) viewModel.onManualUidFocused() },
        ...
        Row(
            ...
            Text(text = " ", style = typography.bodyRegular)
            Text(
