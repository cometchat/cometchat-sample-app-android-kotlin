#!/usr/bin/env bash
#
# run_e2e_tests.sh — one-command E2E runner for sample-app-kotlin
#
#   ./sample-app-kotlin/run_e2e_tests.sh                 # run the whole suite
#   ./sample-app-kotlin/run_e2e_tests.sh <FQ test class> # run one class
#   ./sample-app-kotlin/run_e2e_tests.sh <class#method>  # run one test
#
# Examples:
#   ./sample-app-kotlin/run_e2e_tests.sh com.cometchat.sampleapp.kotlin.e2e.AuthenticationE2ETest
#   ./sample-app-kotlin/run_e2e_tests.sh com.cometchat.sampleapp.kotlin.e2e.realtime.RealTimeMessagingTest#test02_receiveText
#
# What it does:
...
MODULE="sample-app-kotlin"
PKG="com.cometchat.sampleapp.kotlin"
...
# ─── 2b. Push real test media assets ─────────────────────────────────────────
# The media E2E tests attach a file/image/video/audio from /sdcard/Download. The Android-16
# photo picker validates real media (it rejects the in-test echo'd dummy text), so we push the
# real assets committed under src/androidTest/assets. The tests keep any file that already
# exists, so these win over the dummy fallback.
bold "2b. Test media assets → /sdcard/Download (on $SERIAL)"
ASSET_DIR="$SCRIPT_DIR/src/androidTest/assets"
pushed_any=0
for a in test_image.jpg test_document.pdf test_video.mp4 test_audio.mp3; do
  if [ -f "$ASSET_DIR/$a" ]; then
    if "$ADB" -s "$SERIAL" push "$ASSET_DIR/$a" "/sdcard/Download/$a" >/dev/null 2>&1; then
      ok "pushed $a"
      pushed_any=1
      "$ADB" -s "$SERIAL" shell am broadcast -a android.intent.action.MEDIA_SCANNER_SCAN_FILE \
        -d "file:///sdcard/Download/$a" >/dev/null 2>&1 || true
    else
      warn "failed to push $a"
    fi
  else
    warn "$a missing from assets — its media test may fall back to the in-test dummy and fail on A16"
  fi
done
[ "$pushed_any" -eq 0 ] && warn "No media assets pushed."
