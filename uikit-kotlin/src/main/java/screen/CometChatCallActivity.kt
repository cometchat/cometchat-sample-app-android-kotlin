...
class CometChatCallActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private var callerAvatar: Avatar? = null
    ...
    private var userAv: Avatar? = null
    ...
    private fun handleIntent() {
        ...
        if (intent.hasExtra(StringContract.IntentStrings.ID)) {
            val id = intent.getStringExtra(StringContract.IntentStrings.ID)
        }
        ...
        if (!isOngoing) {
            try {
                ...
                isIncoming = intent.type == StringContract.IntentStrings.INCOMING
