...
class CometChatGroupDetailScreenActivity() : AppCompatActivity() {
    private val TAG = "CometChatGroupDetail"
    private lateinit var groupIcon: Avatar
    ...
    private var sharedMediaView: SharedMediaView? = null
    ...
    private fun initComponent() {
        ...
        tvLoadMore!!.setText(String.format(resources.getString(R.string.load_more_members), LIMIT))
            ...
            alert.setTitle(resources.getString(R.string.ongoing_call))
                    ...
                    .setPositiveButton(resources.getString(R.string.join), object : DialogInterface.OnClickListener {
                        override fun onClick(dialog: DialogInterface, which: Int) {
                            Utils.joinOnGoingCall(this@CometChatGroupDetailScreenActivity)
                        }
    ...
    fun initiateGroupCall(recieverID: String?, receiverType: String?, callType: String?) {
        ...
        CometChat.initiateCall(call, object : CallbackListener<Call>() {
            override fun onSuccess(call: Call) {
                Utils.startGroupCallIntent(this@CometChatGroupDetailScreenActivity, (call.callReceiver as Group), call.type, true, call.sessionId)
            }
    ...
    private fun createDialog(title: String, message: String, positiveText: String, negativeText: String, drawableRes: Int) {
        val alert_dialog = MaterialAlertDialogBuilder(this@CometChatGroupDetailScreenActivity,
    ...
    private fun handleIntent() {
        ...
        if (intent.hasExtra(StringContract.IntentStrings.NAME)) {
            gName = intent.getStringExtra(StringContract.IntentStrings.NAME)
        ...
        if (intent.hasExtra(StringContract.IntentStrings.GROUP_DESC)) {
            gDesc = intent.getStringExtra(StringContract.IntentStrings.GROUP_DESC)
        }
        if (intent.hasExtra(StringContract.IntentStrings.GROUP_PASSWORD)) {
            gPassword = intent.getStringExtra(StringContract.IntentStrings.GROUP_PASSWORD)
        }
        ...
        if (intent.hasExtra(StringContract.IntentStrings.MEMBER_COUNT)) {
            tvMemberCount!!.visibility = View.VISIBLE
            groupMemberCount = intent.getIntExtra(StringContract.IntentStrings.MEMBER_COUNT, 0)
            tvMemberCount!!.text = (groupMemberCount).toString() + " Members"
        }
        if (intent.hasExtra(StringContract.IntentStrings.GROUP_TYPE)) {
            groupType = intent.getStringExtra(StringContract.IntentStrings.GROUP_TYPE)
        }
    }
    ...
    private fun openBanMemberListScreen() {
        val intent = Intent(this, CometChatBanMemberScreenActivity::class.java)
    ...
    fun openAdminListScreen(showModerators: Boolean) {
        val intent = Intent(this, CometChatAdminModeratorListScreenActivity::class.java)
        intent.putExtra(StringContract.IntentStrings.GUID, guid)
     ...
     */
    fun addMembers() {
        val intent = Intent(this, CometChatAddMemberScreenActivity::class.java)
        intent.putExtra(StringContract.IntentStrings.GUID, guid)
    ...
    private fun launchUnified() {
        val intent = Intent(this@CometChatGroupDetailScreenActivity, CometChatUnified::class.java)
...
//                        tvModeratorCount!!.text = moderatorCount.toString() + ""
                        if (groupMemberAdapter == null) {
                            groupMemberAdapter = GroupMemberAdapter(this@CometChatGroupDetailScreenActivity, groupMembers!!, ownerId)
...
//                    tvBanMemberCount!!.setText(++count)
    ...
    private fun updateGroupDialog() {
        ...
        val view = LayoutInflater.from(this).inflate(R.layout.update_group, null)
        val avatar: Avatar = view.findViewById(R.id.group_icon)
        ...
        avatar_url.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable) {
                if (!s.toString().isEmpty()) {
                    avatar.visibility = View.VISIBLE
                    Glide.with(this@CometChatGroupDetailScreenActivity).load(s.toString()).into(avatar)
    ...
    private val group: Unit
        private get() {
            CometChat.getGroup((guid)!!, object : CallbackListener<Group>() {
                ...
                override fun onError(e: CometChatException) {
                    Toast.makeText(this@CometChatGroupDetailScreenActivity, "Error:" + e.message, Toast.LENGTH_LONG).show()
                }
            })
        }
