...
class StatusIndicatorPropertyTest : FunSpec({
    ...
    fun createViewModelWithUsers(users: List<User>): CometChatUsersViewModel {
        val fetchUsersUseCase: FetchUsersUseCase = mock {
            onBlocking { invoke(any()) } doReturn Result.success(emptyList<User>())
            on { hasMore() } doReturn false
        }
        val searchUsersUseCase: SearchUsersUseCase = mock()
