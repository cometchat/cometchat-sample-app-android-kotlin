...
class CometChatGroupMembersViewModelTest : FunSpec({
    ...
    // Mockito mocks — avoids subclassing use case classes which triggers
    // NoClassDefFoundError when the SDK AAR classes aren't on the test classpath.
    lateinit var fetchUseCase: FetchGroupMembersUseCase
    lateinit var kickUseCase: KickGroupMemberUseCase
    lateinit var banUseCase: BanGroupMemberUseCase
    lateinit var changeScopeUseCase: ChangeMemberScopeUseCase
    ...
    beforeTest {
        ...
        kickUseCase = mock()
        banUseCase = mock()
        changeScopeUseCase = mock()
        // Default stubs
        whenever(fetchUseCase.hasMore()).thenReturn(true)
    }

    /**
     * Helper: create a fresh set of mocked use cases with default stubs.
     * Use this inside property-based tests where each iteration needs fresh mocks.
     */
    data class MockUseCases(
        val fetch: FetchGroupMembersUseCase,
        val kick: KickGroupMemberUseCase,
        val ban: BanGroupMemberUseCase,
        val changeScope: ChangeMemberScopeUseCase
    )

    fun freshMocks(): MockUseCases {
        val f: FetchGroupMembersUseCase = mock()
        val k: KickGroupMemberUseCase = mock()
        val b: BanGroupMemberUseCase = mock()
        val c: ChangeMemberScopeUseCase = mock()
        whenever(f.hasMore()).thenReturn(true)
        return MockUseCases(f, k, b, c)
    }
    ...
    context("Task 1.7.4.1: Initial state tests") {
        ...
        test("Property 1: Initial state should always be Loading for any new ViewModel instance") {
            checkAll(10, Arb.int(1, 100)) { _ ->
                ...
                val viewModel = CometChatGroupMembersViewModel(
                    fetchGroupMembersUseCase = m.fetch,
                    kickGroupMemberUseCase = m.kick,
                    banGroupMemberUseCase = m.ban,
                    changeMemberScopeUseCase = m.changeScope,
                    enableListeners = false
                )
    ...
    context("Task 1.7.4.2: Fetch state update tests") {
        ...
        test("fetchGroupMembers should update state to Content when members are returned") {
            runTest {
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(mockMembers))
        ...
        test("fetchGroupMembers should update state to Empty when no members are returned") {
            runTest {
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(emptyList()))
        ...
        test("fetchGroupMembers should update state to Error when fetch fails") {
            runTest {
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.failure(exception))
        ...
        test("Property 2: Fetch should consistently update state based on member count") {
            checkAll(20, Arb.int(0, 100)) { memberCount ->
                runTest {
                    ...
                    whenever(m.fetch.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(mockMembers))
    ...
    context("Task 1.7.4.3: Pagination tests") {
        ...
        test("fetchGroupMembers should append members on subsequent calls") {
            runTest {
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(createMockMembers(3, 1)))
                viewModel.fetchGroupMembers()
                advanceUntilIdle()
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(createMockMembers(3, 4)))
                viewModel.fetchGroupMembers()
                advanceUntilIdle()
        ...
        test("hasMore should be false when empty list is returned") {
            runTest {
                ...
                // First fetch: 3 members
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(createMockMembers(3)))
                viewModel.fetchGroupMembers()
                advanceUntilIdle()
    ...
    context("Task 1.7.4.4: Search filtering tests") {
        ...
        test("searchGroupMembers should clear list and fetch with search keyword") {
            runTest {
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(createMockMembers(5)))
                viewModel.fetchGroupMembers()
                advanceUntilIdle()
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(createMockMembers(2)))
    ...
    context("Task 1.7.4.6: Kick member operation tests") {
        ...
        test("kickMember should remove member from list on success") {
            runTest {
                ...
                // Fetch members
                val members = createMockMembers(3)
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(members))
                viewModel.fetchGroupMembers()
                advanceUntilIdle()
                ...
                // Kick member
                whenever(kickUseCase.invoke(any(), any())).thenReturn(Result.success(Unit))
        ...
        test("kickMember should update state to Error on failure") {
            runTest {
                ...
                whenever(kickUseCase.invoke(any(), any())).thenReturn(Result.failure(exception))
                val memberToKick = members.first()
                viewModel.kickMember(memberToKick)
                advanceUntilIdle()
    ...
    context("Task 1.7.4.7: Ban member operation tests") {
        ...
        test("banMember should remove member from list on success") {
            runTest {
                ...
                // Ban member
                whenever(banUseCase.invoke(any(), any())).thenReturn(Result.success(Unit))
        ...
        test("banMember should update state to Error on failure") {
            runTest {
                ...
                whenever(banUseCase.invoke(any(), any())).thenReturn(Result.failure(exception))
                val memberToBan = members.first()
                viewModel.banMember(memberToBan)
                advanceUntilIdle()
    ...
    context("Task 1.7.4.8: Change scope operation tests") {
        ...
        test("changeMemberScope should update member scope on success") {
            runTest {
                ...
                // Change scope
                whenever(changeScopeUseCase.invoke(any(), any(), any())).thenReturn(Result.success(Unit))
        ...
        test("changeMemberScope should update state to Error on failure") {
            runTest {
                ...
                whenever(changeScopeUseCase.invoke(any(), any(), any())).thenReturn(Result.failure(exception))
                val memberToUpdate = members.first()
                val oldScope = memberToUpdate.scope
    ...
    context("Task 1.7.4.11: Error handling tests") {
        ...
        test("multiple consecutive errors should update state correctly") {
            runTest {
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.failure(exception1))
                viewModel.fetchGroupMembers()
                advanceUntilIdle()
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.failure(exception2))
                viewModel.fetchGroupMembers()
                advanceUntilIdle()
    ...
    context("refreshList tests") {
        ...
        test("refreshList should clear members and refetch") {
            runTest {
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(createMockMembers(5, 10)))
    ...
    context("Bugfix: searchGroupMembers and refreshList should call resetRequest") {
        ...
        test("refreshList should call resetRequest exactly once") {
            runTest {
                ...
                // Reset counter after initial setup — clear invocation history
                org.mockito.Mockito.clearInvocations(fetchUseCase)

                // refreshList should call resetRequest
                viewModel.refreshList()
                advanceUntilIdle()

                verify(fetchUseCase, times(1)).resetRequest()
            }
        }
        ...
        test("searchGroupMembers should call resetRequest exactly once per call") {
            runTest {
                ...
                verify(fetchUseCase, times(2)).resetRequest()
            }
        }
        ...
        test("search then clear should return full list (end-to-end bug scenario)") {
            runTest {
                ...
                whenever(fetchUseCase.invoke(any(), any(), anyOrNull())).thenReturn(Result.success(createMockMembers(2, 10)))
                viewModel.searchGroupMembers("a")
                advanceUntilIdle()
                viewModel.members.value.size shouldBe 2
        ...
        test("pagination should NOT call resetRequest") {
            runTest {
                ...
                verify(fetchUseCase, never()).resetRequest()
                viewModel.members.value.size shouldBe 6
            }
        }
    }
