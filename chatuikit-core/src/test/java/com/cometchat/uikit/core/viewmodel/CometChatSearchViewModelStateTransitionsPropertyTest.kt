...
/**
 * Mock Conversation for testing purposes.
 * Since CometChat SDK Conversation has private constructor, we create test doubles.
 */
private class MockConversation(
    val id: String
)

/**
 * Mock BaseMessage for testing purposes.
 * Since CometChat SDK BaseMessage has private constructor, we create test doubles.
 */
private class MockBaseMessage(
    val id: Int
)

/**
 * Test implementation of FetchConversationsUseCase that allows controlling results.
 */
private class TestFetchConversationsUseCase(
    private var result: Result<List<Conversation>> = Result.success(emptyList()),
    private var hasMoreFlag: Boolean = false
) : FetchConversationsUseCase(
    repository = object : com.cometchat.uikit.core.domain.repository.SearchRepository {
        override suspend fun getConversations(request: ConversationsRequest) = result
        override suspend fun getMessages(request: MessagesRequest) = Result.success(emptyList<BaseMessage>())
        override fun hasMoreConversations() = hasMoreFlag
        override fun hasMoreMessages() = false
    }
) {
    fun setResult(newResult: Result<List<Conversation>>) {
        result = newResult
    }
    ...
    override suspend fun invoke(request: ConversationsRequest): Result<List<Conversation>> = result
    override fun hasMore(): Boolean = hasMoreFlag
}

/**
 * Test implementation of FetchMessagesUseCase that allows controlling results.
 */
private class TestFetchMessagesUseCase(
    private var result: Result<List<BaseMessage>> = Result.success(emptyList()),
    private var hasMoreFlag: Boolean = false
) : FetchMessagesUseCase(
    repository = object : com.cometchat.uikit.core.domain.repository.SearchRepository {
        override suspend fun getConversations(request: ConversationsRequest) = Result.success(emptyList<Conversation>())
        override suspend fun getMessages(request: MessagesRequest) = result
        override fun hasMoreConversations() = false
        override fun hasMoreMessages() = hasMoreFlag
    }
) {
    fun setResult(newResult: Result<List<BaseMessage>>) {
        result = newResult
    }
    ...
    override suspend fun invoke(request: MessagesRequest): Result<List<BaseMessage>> = result
    override fun hasMore(): Boolean = hasMoreFlag
}
        ...
        test("search with non-empty conversation results should result in Content state") {
            checkAll(100, Arb.string(1..50), Arb.int(1..10)) { searchText, conversationCount ->
                runTest(testDispatcher) {
                    ...
                    val conversationsUseCase = TestFetchConversationsUseCase(
                        result = Result.success(emptyList()), // Empty because we can't create real objects
                        hasMoreFlag = false
                    )
                    val messagesUseCase = TestFetchMessagesUseCase(
